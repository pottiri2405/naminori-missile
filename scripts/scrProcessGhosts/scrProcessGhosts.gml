function scrProcessGhosts() {
	...
	if(dir == -1){												// if moving left
	    ...
	    c2 = -1;
	    c1 = tilemap_get_at_pixel(_control.map, x, y);				// check the left edge of the sprite
	    c2 = tilemap_get_at_pixel(_control.map,x,y+sprite_height);				// check below the left edge of the sprite, since ghosts dont like heights
	...
	{															// Otherwise, check collision to the right
		...
		c1 = -1													// reset the collision tile checkers
	    c2 = -1;
	    c1 = tilemap_get_at_pixel(_control.map,x+sprite_width,y);					// check the right edge of the sprite
	    c2 = tilemap_get_at_pixel(_control.map,x+sprite_width,y+sprite_height);	// check below the right edge of the sprite, since ghosts dont like heights
