function scrProcessPlayer() {
	...
	if( grav<0 ){								// If jumping check above player
		...
		c1 = tilemap_get_at_pixel(_control.map,x-(sprite_get_width(sprite_index)/2),y);	// left
		c2 = tilemap_get_at_pixel(_control.map,x+(sprite_get_width(sprite_index)/2),y);	// right
		c3 = tilemap_get_at_pixel(_control.map,x,y);										// center
	...
	if((keyboard_check(vk_left)) || (_control.button_down_left == true) || (gamepad_button_check(0,gp_padl)) || (move_left)){				// moving left collisions
	    ...
	    c1 = tilemap_get_at_pixel(_control.map,x-(sprite_get_width(sprite_index)/2),y-1);				// left
	    c3 = tilemap_get_at_pixel(_control.map,x,y-1);													// center
	    if( y&$3f>0 ) c2=tilemap_get_at_pixel(_control.map,x-(sprite_get_width(sprite_index)/2),y+1);	// left below (only check if there is a tile below)
	...
	}else if((keyboard_check(vk_right)) || (_control.button_down_right == true) || (gamepad_button_check(0,gp_padr)) || (move_right)){			// moving right collisions (check with else so that both directions cant be triggered at the same time)
	    dir=1;									// set the correct direction
		...
		c3 = -1;
		// check the points at the bottom of the sprite
	    c1 = tilemap_get_at_pixel(_control.map,x+(sprite_get_width(sprite_index)/2),y-1);				// right
		c3 = tilemap_get_at_pixel(_control.map,x,y-1);													// center
	    if( y&$3f>0 ) c2=tilemap_get_at_pixel(_control.map,x+(sprite_get_width(sprite_index)/2),y+1);	// right below (only check if there is a tile below)
	    if(c1 == 3) || (c2 == 3){																	// if we are intersecting with a box
				x = real(x&$ffffffc0)+_control.tilesize-(sprite_get_width(sprite_index)/2);			// stop the player from moving
	    }
		if(c3 == 2){							// if we are intersecting with a ladder
			can_climb = true;					// flag that we can climb
		} else {								// if we are not a ladder
	...
	if(can_climb){									// if we can climb
		c3 = tilemap_get_at_pixel(_control.map,x,y-1);	// check the bottom center of the sprite
		if(c3 == 2){								// if it is still on a ladder
			if((keyboard_check(vk_up)) || (_control.button_down_up == true) || (gamepad_button_check(0,gp_padu)) || (move_up)){				// and we are pressing up
			...
			if((keyboard_check(vk_down)) || (_control.button_down_down == true) || (gamepad_button_check(0,gp_padd)) || (move_down)){			// or if we are pressing down
			...
			if(climbing){							// if we are climbing
				...
				x = real(x&$ffffffc0)+_control.tilesize-(sprite_get_width(sprite_index)/2);	// make sure that the player stays on the ladder
			}
